...
def draw_boxes_and_label_on_image_cv2(img, class_label_map, class_boxes_map):
    ...
    for c, boxes in class_boxes_map.items():
        for box in boxes:
            ...
            cv2.rectangle(img, (bb_left, bb_top), (bb_width, bb_height), (0, 0, 255), 2)

            text_label = '{}'.format(class_label_map[c])
            (ret_val, base_line) = cv2.getTextSize(text_label, cv2.FONT_HERSHEY_COMPLEX, 1, 1)
            text_org = (bb_left, bb_top - 5)

#            cv2.rectangle(img, (text_org[0] - 15, text_org[1] + base_line - 15),
#                          (text_org[0] + ret_val[0] + 15, text_org[1] - ret_val[1] + 15), unique_color, 2)
            # this rectangle for fill text rect
#            cv2.rectangle(img, (text_org[0] - 15, text_org[1] + base_line -15),
#                          (text_org[0] + ret_val[0] + 15, text_org[1] - ret_val[1] + 15),
#                          unique_color, -1)
            cv2.putText(img, text_label, text_org, cv2.FONT_HERSHEY_DUPLEX, 0.6, (0, 0, 255), 1)
    return img
